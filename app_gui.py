...
@_app.post("/api/rep-img/upload")
async def _rep_img_upload_endpoint(file: UploadFile = _FastAPIFile(...)):
    """대표이미지 수정 다이얼로그 — 파일/클립보드 → 로컬 저장 → 경로+미리보기 URL 반환."""
    import tempfile as _tf_rep
    try:
        _data = await file.read()
        _fname_orig = file.filename or "img.png"
        _ext = (_fname_orig.rsplit(".", 1)[-1] or "png").lower()
        _ext = _ext if _ext in ("jpg", "jpeg", "png", "webp") else "png"
        _tmp = _tf_rep.NamedTemporaryFile(
            delete=False, suffix=f".{_ext}", dir=str(_IMG_ROOT), prefix="repimg_"
        )
        _tmp.write(_data)
        _tmp.close()
...
async def _process_entry(
    ...
    try:
            ...
            try:
                ...
                if _gc_cats:
                    ...
                    _cat_primary_prompt = (
                        ...
                        "- '엔진오일세트'는 여러 상품을 묶어 파는 세트 상품 전용 카테고리입니다. "
                        "1L/4L/6L 등 단일 용량 오일 1개 상품은 세트가 아니므로 이 카테고리를 고르지 마세요 — "
                        "제목에 가솔린/디젤 표기가 있으면 그에 맞춰, 표기가 없거나 겸용(LPG 등)이면 "
                        "'휘발유/가솔린'을 선택하세요.\n"
                    )
        ...
        _custom_img = getattr(entry, "custom_image_path", "") or ""
        ...
        if entry.single_mode:
            ...
            _single_main = ""
            _single_src = _src_img_path  # custom 이미지 우선
            if _single_src and Path(_single_src).exists():
                _single_main = await loop.run_in_executor(
                    None, lambda p=_single_src: upload_file(p)
                )
                if _single_main:
        ...
        with ui.row().classes("w-full gap-4 items-start"):
            ...
            with ui.column().classes("gap-4 flex-1 min-w-0"):
                ...
                with ui.card().classes("shadow-sm w-full"):
                    ...
                    def _parse_num_range(s: str) -> set:
                        """'1,3,5-8' → {1,3,5,6,7,8}. 빈 문자열 → 빈 set (전체)."""
                        result = set()
                    ...
                    with ui.card().classes("w-full shadow-sm mb-1").style("background:#f8fafc"):
                        with ui.card_section().classes("py-2"):
                            ...
                            # ── 번호 선택 행 ───────────────────────────────
                            with ui.row().classes("items-center gap-2 mb-2"):
                                ui.label("번호").classes("text-xs text-slate-500 font-semibold").style("min-width:44px")
                                _bulk_num_inp = ui.input(
                                    placeholder="예: 9  /  1,3,5-8  /  빈칸=전체"
                                ).props("dense outlined clearable").style("min-width:240px")
                                ui.label("(큐 번호 기준, 빈칸이면 브랜드·카테고리 조건으로 전체 적용)").classes("text-xs text-slate-400")
                            ...
                            with ui.row().classes("justify-end mt-2"):
                                async def _apply_bulk_change():
                                    num_str  = (_bulk_num_inp.value or "").strip()
                                    num_set  = _parse_num_range(num_str)  # 빈 set = 전체
                                    ...
                                    for _qi, _e in enumerate(_global_queue, 1):
                                        ...
                                        if not num_set:
                                            ...
                                            if old_b and old_cid:
                                                if not b_match and not c_match:
                                                    continue
                                            elif old_b and not b_match:
                                                continue
