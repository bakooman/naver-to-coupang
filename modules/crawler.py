...
class NaverStoreCrawler:
    ...
    async def crawl(self, url: str) -> ProductData:
        ...
        if "brand.naver.com" in url:
            ...
            if raw_ss:
                node_ss = self._find_product_node(raw_ss)
                if self._get_name(node_ss) != "Unknown" and self._get_price(node_ss) > 0:
                    ...
                    return product
                print(f"[Crawler] smartstore 동등 URL: 존재하지 않는 상품(빈 페이지) → brand URL 직접 시도")
            else:
