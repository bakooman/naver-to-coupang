...
## 🚨 절대 하면 안 되는 것 (사무실 PC 포함)

### 가격변동알림 데이터 — 절대 초기화/삭제 금지
- **`data/price_watch.json`** — 가격 감시 목록 (~550개 상품). **절대 덮어쓰기/삭제/초기화 금지.**
- 이 파일은 사용자가 수백 개 상품을 등록한 핵심 데이터임. 잘못 건드리면 전부 날아감.
- 코드 수정 중 이 파일에 관련된 작업이 없으면 절대 언급도 하지 말 것.
- "초기화할까요?", "리셋할까요?", "샘플 데이터로 교체할까요?" 같은 말 절대 금지.
- 서버에서 직접 수정이 필요한 경우 → **반드시 사용자 명시적 지시가 있을 때만**.

### 앱 실행 방법 — 로컬 실행 금지, 웹으로 접속
- **`run.bat` 실행 금지** — 로컬에서 앱을 따로 띄울 필요 없음.
- **"python app_gui.py 실행하세요", "uvicorn 실행하세요" 같은 말 하지 말 것.**
- 앱은 이미 서버(115.68.223.177)에서 24시간 실행 중.
- **접속 방법: 브라우저에서 [https://bakoo-mm.com](https://bakoo-mm.com) 열면 끝.**
  - 수집 탭: `https://bakoo-mm.com/` (메인 페이지)
  - 가격감시 탭: `https://bakoo-mm.com/monitor`
  - 가격수정 탭: `https://bakoo-mm.com/price-fix`
...
## General Coding Guidelines (Andrej Karpathy)
